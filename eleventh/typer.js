...
function restartGame(){
    $('.submitName').click((e)=>startGame(e.target));
...
function startGame(target){
    console.log($(target).siblings('.nameVal').val())
    $(document).off('keypress');
    let name = $(target).siblings('.nameVal').val();
...
class Typer{
    ...
    selectContent(){
        if($('[data-menu-game]').hasClass('active')){
            ...
            $('.submitName').on('click', (e)=>startGame(e.target));
            $('#tutorial').hide();
            $('#results').hide();
            $('#nameContainer').show();
            $('#startingLength').val(this.wordsInGame);
            console.log(this.name);
            $('#name').val(this.name);
            $('#level').val(this.startingWordLength);
        }
    }
